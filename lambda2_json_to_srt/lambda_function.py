...
#bucket_name_original_srt = "transcribe.json.conygre.com"
#bucket_name_translated_srt = "transcribe.srt.conygre.com"
...
def save_original_and_translated(json_file_name,source_language_code,target_language_code):
    output_bucket_name = os.environ["OUTPUT_BUCKET"] if os.environ["OUTPUT_BUCKET"] else "transcribe.srt.conygre.com"
    ...
    # SRT file for original and translated language are saved in the same bucket
    basefilename_original = os.path.basename(srt_filename_original)
    s3.upload_file(srt_filename_original, output_bucket_name, basefilename_original)
    
    print("uploaded original: ",basefilename_original)

    basefilename_translated = os.path.basename(srt_filename_translated)
    s3.upload_file(srt_filename_translated, output_bucket_name, basefilename_translated)
...
def lambda_handler(event, context):
    ...
    s3.download_file(input_bucket_name, json_file_name, output_file_path)
